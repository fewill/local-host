...
- `../opn-support` — opn-support-poller, gh-event-poller (system); opn-support-rtp-funding-watcher (user; rtp-funding-*.json and waiting-*.json drops → support.opn.inc cases); opn-support-dmarc-import.timer/service (user, every 30 min; DMARC reports from the inbox → ../dmarc-reports); opn-support-intake-canary.timer/service (user, daily 08:00; probe email → support@opn.inc, confirms the round trip and dismisses itself as noise). sms_inbound_poller and the mailbox-import timer/service were retired 2026-09-09
