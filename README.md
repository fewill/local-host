...
| `opn-support-intake-canary.timer` | Timer | triggers opn-support-intake-canary.service weekday mornings at 6am ET |
| `opn-support-intake-canary.service` | Service | verifies pending probe emails to support@opn.inc and dismisses confirmed ones as noise every run; skips sending a new probe if a real inbound case landed in the last 24h, otherwise sends one; alerts #ops-support starting ~20h after a probe goes missing, repeating each weekday morning it's still missing, final alert and give-up at 3 days (oneshot) — inactive (dead) is normal; runs only when triggered by timer |
